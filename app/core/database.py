...
class Database:
    ...
    async def connect(self):
        ...
        try:
            ...
            print(f"✅ Connected to PostgreSQL at {host}:{port}/{db_name}")
    ...
    async def initialize_database(self):
        ...
        try:
            ...
            if FASTAPI_ENV == "production":
                ...
                logger.info("🔧 Using Supabase-compatible SQL for local PostgreSQL (testing mode)")
            else:
                # For local development, use traditional init.sql
                init_sql_path = sql_base_path / "init.sql"
                logger.info("🔧 Using local PostgreSQL init.sql")
            ...
            if init_sql_path.exists():
                ...
                logger.warning(f"⚠️  SQL file not found at {init_sql_path}")
    ...
    def _adapt_supabase_sql_for_postgres(self, sql_content: str) -> str:
        ...
        adapted_sql = '\n'.join(filtered_lines)
        logger.info("🔧 Adapted Supabase SQL for PostgreSQL compatibility")
        return adapted_sql
    ...
    async def run_migrations(self):
        ...
        try:
            ...
            if migration_count > 0:
                print(f"📦  Applied {migration_count} database migrations")
...
async def init_database():
    ...
    if not success:
        logger.warning("⚠️  Running without database - demo mode only")
    return success
